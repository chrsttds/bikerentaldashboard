...
mindate = day_df['dteday'].min().date()
maxdate = day_df['dteday'].max().date()
startdate = st.sidebar.date_input('Start date', mindate)
enddate = st.sidebar.date_input('End date', maxdate)
filtered_df = day_df[(day_df['dteday'].dt.date >= startdate) & (day_df['dteday'].dt.date <= enddate)]
...
sns.scatterplot(x='temp', y='cnt', data=filtered_df)
ax.set_title('Distribusi antara temperatur dan jumlah user')
ax.set_xlabel('temperatur')
ax.set_ylabel('jumlah user')
st.pyplot(fig)

st.write("""Diatas adalah hasil scatter plot dari distribusi temperatur dan jumlah user . Anda bisa 
menggunakan sidebar untuk melihat data dalam rentang tanggal tertentu""")
